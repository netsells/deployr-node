...
const handler = async (context) => {
    if (!(process.getuid && process.getuid() === 0)) {
        throw new Error('Must be ran as super user. Run `sudo su` before running this command');
    }
    ...
    context.info([
        'Run the following command to download the generated SSH key:',
    ...
    context.success([
        ...
        '',
        chalk.red('Make sure you exit super user using `exit`'),
    ].join('\n  '));
};
